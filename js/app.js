 ...
 *****************************************************************************
 */
let timeLineSlider = d3.select("#timeLineSlider");
  ...
  switch (mediaImg.id) {
    ...
    case "mediaPlayPause":
      ...
      if (mediaControls.mediaIsPlaying) {
        $("#mediaPlayPause").attr(
          "src",
          "images/ic_play_arrow_black_24dp_2x.png"
        );
...
axisControls.yAxisOptionsChanged = function() {
  ...
  let value = $("#y-axis-options-select").val();
  console.log(value);
  ...
  yScale.domain(
    d3
      .extent(chartProperties.mainData, function(d) {
...
axisControls.xAxisOptionsChanged = function() {
  ...
  let value = $("#x-axis-options-select").val();
  console.log(value);
  chartProperties.xAxisCurrentValue = value;
  axisControls.setDefaultThresholdsForAxes();

  xScale.domain(
    d3.extent(chartProperties.mainData, function(d) {
...
axisControls.yAxisChangeThresholdValue = function() {
  ...
  let minY = $("#input-y-axis-lower-threshold").val();
  let maxY = $("#input-y-axis-upper-threshold").val();
  if (minY == "") {
...
axisControls.xAxisChangeThresholdValue = function() {
  ...
  let minX = $("#input-x-axis-lower-threshold").val();
  let maxX = $("#input-x-axis-upper-threshold").val();
  if (minX == "") {
...
axisControls.setDefaultThresholdsForAxes = function() {
  $("#input-y-axis-upper-threshold")
    .val(CONSTANTS.rangeValues[chartProperties.yAxisCurrentValue][1])
    .attr(
      "placeholder",
      CONSTANTS.rangeValues[chartProperties.yAxisCurrentValue][1]
    );
  $("#input-y-axis-lower-threshold")
    .val(CONSTANTS.rangeValues[chartProperties.yAxisCurrentValue][0])
    .attr(
      "placeholder",
      CONSTANTS.rangeValues[chartProperties.yAxisCurrentValue][0]
    );
  $("#input-x-axis-upper-threshold")
    .val(CONSTANTS.rangeValues[chartProperties.xAxisCurrentValue][1])
    .attr(
      "placeholder",
      CONSTANTS.rangeValues[chartProperties.xAxisCurrentValue][1]
    );
  $("#input-x-axis-lower-threshold")
    .val(CONSTANTS.rangeValues[chartProperties.xAxisCurrentValue][0])
    .attr(
      "placeholder",
      CONSTANTS.rangeValues[chartProperties.xAxisCurrentValue][0]
    );
};
...
axisControls.showRiskRangesForYAxis = function() {
  ...
  axisControls.yAxisUpperRiskRangeBox
    .append("rect")
  ...
  axisControls.yAxisLowerRiskRangeBox
    .append("rect")
...
axisControls.showRiskRangesForXAxis = function() {
  ...
  axisControls.xAxisUpperRiskRangeBox
    .append("rect")
  ...
  axisControls.xAxisLowerRiskRangeBox
    .append("rect")
...
tooltipFunctions.onMouseOver = function(d) {
  ...
  tooltip
    .html(tooltipFunctions.getHtmlStringForToolTip(d))
...
tooltipFunctions.getHtmlStringForToolTip = function(d_in) {
  ...
  let html = '<div class="heading-tooltip"><p>Date: <b>' + d.Date;
  ...
  html =
    ...
    d.BMI.toPrecision(4) +
    " <br/>" +
    d.Size.toPrecision(4) +
    " <br/>" +
    d.Weight.toPrecision(4) +
    "<br/>" +
    d.RR_syst.toPrecision(4) +
    "<br/>" +
    d.RR_diast.toPrecision(4) +
    "<br/>" +
    d.Nikotin +
    ...
    d.NBZ.toPrecision(4) +
    " <br/>" +
    d.HbA1c.toPrecision(4) +
    " <br/>" +
    d.Chol.toPrecision(4) +
    "<br/>" +
    d.TG.toPrecision(4) +
    "<br/>" +
    d.Crea.toPrecision(2) +
    "<br/>" +
    d.Protein_in_urine +
...
tooltipFunctions.appendLineChart = function(d) {
  ...
  chartProperties.mainData.forEach(function(x) {
    if (
      x.ID == d.ID &&
      x.opacity == 1 &&
      x[chartProperties.xAxisCurrentValue] != 0 &&
      x[chartProperties.yAxisCurrentValue] != 0
    ) {
  ...
  chartProperties.lineChartGroup
    .append("path")
...
chartProperties.width =
  chartProperties.outerWidth -
  chartProperties.margin.left -
  chartProperties.margin.right;
chartProperties.height =
  chartProperties.outerheight -
  chartProperties.margin.top -
  chartProperties.margin.bottom;
...
let ygridlinesGroup = svg
  .append("g")
  .attr("class", "grid")
  .call(ygridlines);

let xgridlinesGroup = svg
  .append("g")
  .attr("class", "grid")
  .call(xgridlines);
...
function beforeDrawingChart(data) {
  ...
  chartProperties.nestedDataByDate = d3
    .nest()
    .key(function(d) {
      return d.Date;
    })
  ...
  setDateOptionsForSlider(
    chartProperties.dateArray[0],
    chartProperties.dateArray[chartProperties.dateArray.length - 1]
  );
...
function setDateOptionsForSlider(min_date, max_date) {
  ...
  chartProperties.days = d3.timeDay.range(
    new Date(min_date),
    new Date(max_date)
  );
  ...
  $("#sliderDateIndicatorText").text(
    "Currently Visible Time:" +
      new Date(min_date).toISOString().substring(0, 10)
  );
...
function timeSliderOptionChanged(x) {
  ...
  let d = chartProperties.days[x];
  // console.log(d.toISOString().substring(0, 10));
  mediaControls.currentSliderValue = chartProperties.days.indexOf(d);
  $("#sliderDateIndicatorText").text(
    "Currently Visible Time:" + d.toISOString().substring(0, 10)
  );
  for (i = 0; i < chartProperties.nestedDataByDate.length; i++) {
    if (
      chartProperties.nestedDataByDate[i].key ==
...
function drawChart(data) {
  ...
  let circles = svg.selectAll("circle").data(chartProperties.data);
  // .filter(function(d){
  ...
  let squares = squaresGroup.selectAll("rect").data(chartProperties.data);
...
marksFunctions.circleSizeOptions = function(d) {
  if (
    d[chartProperties.xAxisCurrentValue] == 0 ||
    d[chartProperties.yAxisCurrentValue] == 0
  ) {
    return 0;
  }
...
marksFunctions.squareSizeOptions = function(d) {
  if (
    d[chartProperties.xAxisCurrentValue] == 0 ||
    d[chartProperties.yAxisCurrentValue] == 0 ||
    marksControls.shapeOption == "none"
  ) {
    return 0;
  }
  if (marksControls.shapeOption == "none") {
 ...
 **************************************************************************************
 */
let colorsLegendSvg = d3.select("#colorsLegendSvg");
colorsLegendData = [
  {
...
// shapesLegendSvg
let shapesLegendSvg = d3.select("#shapesLegendSvg");
shapesLegendSvg
  .selectAll("circle")
  .data(["1"])
  .enter()
